...
x._snapshot = function (tree, optionalMessage, match, snapshotStateGetter) {
	...
	const result = toMatchSnapshot.call(context, JSON.stringify(tree));
	...
	const expected = JSON.parse(result.expected.slice(1).slice(0, -1));
	Object.defineProperty(expected, '$$typeof', {value: Symbol.for('react.test.json')});

	test(result.pass, create(tree, expected, 'snapshot', message, x.snap));
};
